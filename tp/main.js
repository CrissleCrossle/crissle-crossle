...
function loadGame(game) {
  ...
  for (let c = 0; c < game.length; c += WORD_LENGTH) {
    for (let letter = 0; letter < WORD_LENGTH; letter++) {
      let index = c+letter
      if (!game[index]) {
        setTimeout(() => {
          loading = false
    ...
    setTimeout(()=>{submitGuess(false, !game[c+WORD_LENGTH])}, (c*100)+100)
  }
}
...
function saveGame() {
  ...
  window.localStorage.setItem("tp-game", JSON.stringify(newData))
}
...
function getRevealTiles() {
  let filledLetters = guessGrid.querySelectorAll("[data-letter]").length-WORD_LENGTH
  let revealTiles = []

  for (let i = filledLetters; i < filledLetters+WORD_LENGTH; i++) {
